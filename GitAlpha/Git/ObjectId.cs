...
public sealed class ObjectId : IEquatable<ObjectId>, IComparable<ObjectId>
{
	...
	public static bool TryParseAsciiHexBytes(byte[] bytes, int index,
		[NotNullWhen(returnValue: true)] out ObjectId? objectId)
	{
	...
	public static bool TryParseAsciiHexBytes(ArraySegment<byte> bytes, int index,
		[NotNullWhen(returnValue: true)] out ObjectId? objectId)
	{
	...
	/// <param name="objectId">The parsed <see cref="ObjectId"/>.</param>
	/// <returns><c>true</c> if parsing succeeded, otherwise <c>false</c>.</returns>
	public static bool TryParseAsciiHexBytes(ArraySegment<byte> bytes,
		[NotNullWhen(returnValue: true)] out ObjectId? objectId)
		=> TryParseAsciiHexReadOnlySpan(bytes.AsSpan(), out objectId);

	public static bool TryParseAsciiHexReadOnlySpan(in ReadOnlySpan<byte> array,
		[NotNullWhen(returnValue: true)] out ObjectId? objectId)
	{
	...
	[Pure]
	public static bool IsValidPartial(string s, int minLength) =>
		s.Length >= minLength && s.Length <= Sha1CharCount && IsValidCharacters(s);
